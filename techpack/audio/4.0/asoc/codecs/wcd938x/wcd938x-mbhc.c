...
static int wcd938x_mbhc_request_irq(struct snd_soc_codec *codec,
...
static void wcd938x_mbhc_irq_control(struct snd_soc_codec *codec,
				   int irq, bool enable)
{
	struct wcd938x_priv *wcd938x = dev_get_drvdata(codec->dev);
...
static int wcd938x_mbhc_free_irq(struct snd_soc_codec *codec,
			       int irq, void *data)
{
	struct wcd938x_priv *wcd938x = dev_get_drvdata(codec->dev);
...
static void wcd938x_mbhc_clk_setup(struct snd_soc_codec *codec,
				 bool enable)
{
	if (enable)
		snd_soc_update_bits(codec, WCD938X_MBHC_NEW_CTL_1,
				    0x80, 0x80);
	else
		snd_soc_update_bits(codec, WCD938X_MBHC_NEW_CTL_1,
				    0x80, 0x00);
...
static int wcd938x_mbhc_btn_to_num(struct snd_soc_codec *codec)
{
	return snd_soc_read(codec, WCD938X_ANA_MBHC_RESULT_3) & 0x7;
}

static void wcd938x_mbhc_mbhc_bias_control(struct snd_soc_codec *codec,
					 bool enable)
{
	if (enable)
		snd_soc_update_bits(codec, WCD938X_ANA_MBHC_ELECT,
				    0x01, 0x01);
	else
		snd_soc_update_bits(codec, WCD938X_ANA_MBHC_ELECT,
				    0x01, 0x00);
...
static void wcd938x_mbhc_program_btn_thr(struct snd_soc_codec *codec,
				       ...
				       int num_btn, bool is_micbias)
{
	...
	for (i = 0; i < num_btn; i++) {
		...
		snd_soc_update_bits(codec, WCD938X_ANA_MBHC_BTN0 + i,
				    0xFC, vth << 2);
...
static bool wcd938x_mbhc_hph_pa_on_status(struct snd_soc_codec *codec)
{
	return (snd_soc_read(codec, WCD938X_ANA_HPH) & 0xC0) ?
									true : false;
}
...
static int wcd938x_mbhc_request_micbias(struct snd_soc_codec *codec,
					int micb_num, int req)
{
	int ret = 0;

	ret = wcd938x_micbias_control(codec, micb_num, req, false);

	return ret;
}

static void wcd938x_mbhc_micb_ramp_control(struct snd_soc_codec *codec,
					   bool enable)
{
	if (enable) {
		snd_soc_update_bits(codec, WCD938X_ANA_MICB2_RAMP,
				    0x1C, 0x0C);
		snd_soc_update_bits(codec, WCD938X_ANA_MICB2_RAMP,
				    0x80, 0x80);
	} else {
...
{
	struct wcd938x_pdata *pdata = dev_get_platdata(codec->dev);
	int rc, micb_mv;
	...
	rc = wcd938x_mbhc_micb_adjust_voltage(codec, micb_mv, MIC_BIAS_2);
...
static void wcd938x_mbhc_zdet_ramp(struct snd_soc_codec *codec,
				 ...
				 int32_t *zl, int32_t *zr, s16 *d1_a)
{
	...
	snd_soc_update_bits(codec, WCD938X_MBHC_NEW_ZDET_ANA_CTL,
				0x70, zdet_param->ldo_ctl << 4);
	snd_soc_update_bits(codec, WCD938X_ANA_MBHC_BTN5, 0xFC,
			    zdet_param->btn5);
	snd_soc_update_bits(codec, WCD938X_ANA_MBHC_BTN6, 0xFC,
			    zdet_param->btn6);
	snd_soc_update_bits(codec, WCD938X_ANA_MBHC_BTN7, 0xFC,
			    zdet_param->btn7);
	snd_soc_update_bits(codec, WCD938X_MBHC_NEW_ZDET_ANA_CTL,
				0x0F, zdet_param->noff);
	snd_soc_update_bits(codec, WCD938X_MBHC_NEW_ZDET_RAMP_CTL,
					  ...
					  uint32_t *zr)
{
	...
	reg0 = snd_soc_read(codec, WCD938X_ANA_MBHC_BTN5);
	reg1 = snd_soc_read(codec, WCD938X_ANA_MBHC_BTN6);
	reg2 = snd_soc_read(codec, WCD938X_ANA_MBHC_BTN7);
	reg3 = snd_soc_read(codec, WCD938X_MBHC_CTL_CLK);
	reg4 = snd_soc_read(codec, WCD938X_MBHC_NEW_ZDET_ANA_CTL);

	if (snd_soc_read(codec, WCD938X_ANA_MBHC_ELECT) & 0x80) {
		is_fsm_disable = true;
		regmap_update_bits(wcd938x->regmap,
	...
	wcd938x_mbhc_zdet_ramp(codec, zdet_param_ptr, &z1L, NULL, d1);
...
left_ch_impedance:
	if ((z1L == WCD938X_ZDET_FLOATING_IMPEDANCE) ||
		...
		*zl = z1L/1000;
		wcd938x_wcd_mbhc_qfuse_cal(codec, zl, 0);
	}
...
right_ch_impedance:
	if ((z1R == WCD938X_ZDET_FLOATING_IMPEDANCE) ||
		...
		*zr = z1R/1000;
		wcd938x_wcd_mbhc_qfuse_cal(codec, zr, 1);
	}
	...
	snd_soc_update_bits(codec, WCD938X_HPH_R_ATEST, 0x02, 0x02);
	snd_soc_update_bits(codec, WCD938X_HPH_PA_CTL2, 0x40, 0x01);
	if (*zl < (WCD938X_ZDET_VAL_32/1000))
		wcd938x_mbhc_zdet_ramp(codec, &zdet_param[0], &z1Ls, NULL, d1);
	else
		wcd938x_mbhc_zdet_ramp(codec, &zdet_param[1], &z1Ls, NULL, d1);
	snd_soc_update_bits(codec, WCD938X_HPH_PA_CTL2, 0x40, 0x00);
	snd_soc_update_bits(codec, WCD938X_HPH_R_ATEST, 0x02, 0x00);
	z1Ls /= 1000;
	wcd938x_wcd_mbhc_qfuse_cal(codec, &z1Ls, 0);
...
zdet_complete:
	...
	snd_soc_write(codec, WCD938X_ANA_MBHC_BTN7, reg2);
	...
	snd_soc_write(codec, WCD938X_MBHC_NEW_ZDET_ANA_CTL, reg4);
	snd_soc_write(codec, WCD938X_MBHC_CTL_CLK, reg3);
	if (is_fsm_disable)
		regmap_update_bits(wcd938x->regmap,
				   WCD938X_ANA_MBHC_ELECT, 0x80, 0x80);
}

static void wcd938x_mbhc_gnd_det_ctrl(struct snd_soc_codec *codec,
			bool enable)
{
	if (enable) {
		snd_soc_update_bits(codec, WCD938X_ANA_MBHC_MECH,
				    0x02, 0x02);
		snd_soc_update_bits(codec, WCD938X_ANA_MBHC_MECH,
				    0x40, 0x40);
	} else {
...
static void wcd938x_mbhc_hph_pull_down_ctrl(struct snd_soc_codec *codec,
					  bool enable)
{
	if (enable) {
		snd_soc_update_bits(codec, WCD938X_HPH_PA_CTL2,
				    0x40, 0x40);
		snd_soc_update_bits(codec, WCD938X_HPH_PA_CTL2,
				    0x10, 0x10);
	} else {
...
static void wcd938x_mbhc_moisture_config(struct wcd_mbhc *mbhc)
{
	...
	if ((mbhc->moist_rref == R_OFF) ||
		...
		snd_soc_update_bits(codec, WCD938X_MBHC_NEW_CTL_2,
				    0x0C, R_OFF << 2);
		return;
	}
...
static bool wcd938x_mbhc_get_moisture_status(struct wcd_mbhc *mbhc)
{
	 ...
	 * detection.
	 */
	if ((snd_soc_read(codec, WCD938X_MBHC_NEW_CTL_2) & 0x0C))
		goto done;

	wcd938x_mbhc_moisture_detect_en(mbhc, true);
	/* Read moisture comparator status */
	ret = ((snd_soc_read(codec, WCD938X_MBHC_NEW_FSM_STATUS)
				& 0x20) ? 0 : 1);
...
{
	if (bcs_enable)
		wcd938x_disable_bcs_before_slow_insert(mbhc->codec, false);
	else
		wcd938x_disable_bcs_before_slow_insert(mbhc->codec, true);
}
 ...
 */
int wcd938x_mbhc_hs_detect(struct snd_soc_codec *codec,
 ...
 * @codec: handle to snd_soc_codec *
 */
void wcd938x_mbhc_hs_detect_exit(struct snd_soc_codec *codec)
{
	struct wcd938x_priv *wcd938x = NULL;
	struct wcd938x_mbhc *wcd938x_mbhc = NULL;
			...
			struct snd_soc_codec *codec)
{
	...
	wcd938x_mbhc_hs_detect_exit(codec);
				...
				struct fw_info *fw_data)
{
	...
	wcd938x_mbhc = devm_kzalloc(codec->dev, sizeof(struct wcd938x_mbhc),
				    GFP_KERNEL);
	if (!wcd938x_mbhc)
		return -ENOMEM;
	...
	return 0;
err:
	devm_kfree(codec->dev, wcd938x_mbhc);
	return ret;
}
 ...
 * @codec: handle to snd_soc_codec *
 */
void wcd938x_mbhc_deinit(struct snd_soc_codec *codec)
{
